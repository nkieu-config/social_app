...
const ProfilePage = () => {
  ...
  const fetchUserPosts = async () => {
    try {
      const response = await axios.get(`http://localhost:3001/api/posts/user/${username}?page=${page}&limit=10`);
  ...
  const checkFollowStatus = async () => {
    ...
    try {
      ...
      const followResponse = await axios.get(`http://localhost:3001/api/follows/${userId}/check`);
  ...
  const handleFollow = async () => {
    ...
    try {
      if (isFollowing) {
        await axios.delete(`http://localhost:3001/api/follows/${user.id}`);
        setIsFollowing(false);
      } else {
        await axios.post('http://localhost:3001/api/follows', { followingId: user.id });
        setIsFollowing(true);
      }
