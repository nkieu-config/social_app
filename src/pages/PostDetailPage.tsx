...
const PostDetailPage = () => {
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(`http://localhost:3001/api/comments/post/${postId}?page=${page}&limit=10`);
  ...
  const checkLikeStatus = async () => {
    try {
      const response = await axios.get(`http://localhost:3001/api/likes/${postId}/check`);
  ...
  const toggleLike = async () => {
    try {
      if (isLiked) {
        await axios.delete(`http://localhost:3001/api/likes/${postId}`);
        setLikesCount(prev => prev - 1);
      } else {
        await axios.post('http://localhost:3001/api/likes', { postId });
        setLikesCount(prev => prev + 1);
      }
      setIsLiked(!isLiked);
    } catch (error) {
  ...
  return (
    <div className="p-4 pb-20 md:pb-4">
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6">
        <div className="flex justify-between">
          <Link to={`/profile/${post.user.username}`} className="flex items-center">
            ...
            <div>
              <h3 className="font-medium">{post.user.fullName || post.user.username}</h3>
      ...
      {/* Comments */}
      <div>
        <h2 className="text-lg font-medium mb-4">Comments ({post._count.comments})</h2>
